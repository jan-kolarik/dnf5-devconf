...
function loadReposDiagramInit() {
    loadReposLines = [];
    
    $('.load-repos-diagram-line').each(function() {
        var connections = this.id.split("_");
        ...
        loadReposLines.push(loadReposLine);
    });

    loadReposLines.forEach(line => {
        line.show('draw', {
            duration: 2000,
            timing: [0.58, 0, 0.42, 1]
        });
...
function roadmapDiagramInit() {
    roadmapLines = [];
    
    $('.roadmap-diagram-line').each(function() {
        var connections = this.id.split("_");
